...
void drawSimulation(sf::RenderWindow& window, const sf::Font& font,
    ...
    if (toolMenuAnimation > 0.01f) {
        ...
        // ✅ UTILISER LES COULEURS RÉELLES DES BOUTONS (définies dans main.cpp)
        std::vector<ButtonStyle> buttonStyles;
        for (size_t idx = 0; idx < simButtons.size(); ++idx) {
            const auto& btn = simButtons[idx];
            // Couleur d'icône : version plus claire de normalColor
            sf::Color iconCol(
                static_cast<sf::Uint8>(std::min(255, btn.normalColor.r + 80)),
                static_cast<sf::Uint8>(std::min(255, btn.normalColor.g + 80)),
                static_cast<sf::Uint8>(std::min(255, btn.normalColor.b + 80)),
                255
            );
            // Texte : noir si couleur claire, blanc sinon
            int brightness = btn.normalColor.r + btn.normalColor.g + btn.normalColor.b;
            sf::Color textCol = (brightness > 450) ? sf::Color(30, 30, 50) : sf::Color(255, 255, 255);

            buttonStyles.push_back({ btn.normalColor, iconCol, btn.hoverColor, textCol });
        }
