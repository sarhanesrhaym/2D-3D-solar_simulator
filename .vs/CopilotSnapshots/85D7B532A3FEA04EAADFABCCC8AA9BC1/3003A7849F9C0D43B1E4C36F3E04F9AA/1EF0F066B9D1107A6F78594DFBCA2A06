...
struct Meteorite {
    ...
    void spawn(unsigned int windowWidth, unsigned int windowHeight) {
        ...
        if (side == 0) {
            ...
            position.y = -50.f;
            velocity.x = static_cast<float>((std::rand() % 80) - 40);  // ✅ Parenthèses
        ...
        rotationSpeed = static_cast<float>((std::rand() % 300) - 150);  // ✅ Parenthèses
...
void drawTeamInfo(sf::RenderWindow& window, const sf::Font& font,
    ...
    std::vector<std::tuple<std::string,  std::string>> teamMembers = {
        {"MIDINI HAFSA",  "Coordination, Architecture, Developpement principal"},
        {"SANGARE NFALY","Algorithmes, Simulation, Calculs scientifiques"},
        {"SARHANE AYMANE",  "Interface, Experience utilisateur, Graphismes"},
        {"BAJOUDI ALAE",  "Optimisation, Fonctionnalites, Tests"},
        {"EL GHAYATI SARA","Documentation, Tests, Validation"}
    };
...
void drawSimulation(sf::RenderWindow& window, const sf::Font& font,
    ...
    EducationalMode* educationalMode) {  // ✅ NOUVEAU
    ...
    // ✅ NOUVEAU : Dessiner les marqueurs de missions spatiales
    if (missionTimeline && showMissionPanel) {
        missionTimeline->drawMissionMarkers(window, planets, Constants::SUN_CENTER);
    }

    // ===== PANNEAU D'INFO FLOTTANT ATTACHÉ À LA PLANÈTE =====
    if (cameraFollowMode && followedPlanetIndex >= 0 &&
        ...
        // ===== PANNEAU FLOTTANT À CÔTÉ DE LA PLANÈTE =====
        float panelW = 160.f;
        float panelH = 120.f;
        ...
        // Fond du panneau - COULEURS FORTES
    ...
    if (toolMenuAnimation > 0.01f) {
        float menuHeight = static_cast<float>(simButtons.size()) * 40.f + 20.f;
        float animatedHeight = menuHeight * toolMenuAnimation;

        sf::RectangleShape menuBg(sf::Vector2f(130.f, animatedHeight));
        menuBg.setPosition(1065.f, 155.f);
        menuBg.setFillColor(sf::Color(15, 25, 45, static_cast<sf::Uint8>(220 * toolMenuAnimation)));
        menuBg.setOutlineThickness(2.f);
        menuBg.setOutlineColor(sf::Color(60, 120, 180, static_cast<sf::Uint8>(180 * toolMenuAnimation)));
        window.draw(menuBg);

        for (size_t i = 0; i < simButtons.size(); ++i) {
            float buttonDelay = static_cast<float>(i) * 0.05f;
            float buttonAlpha = std::max(0.f, std::min(1.f, (toolMenuAnimation - buttonDelay) * 2.f));
            if (buttonAlpha > 0.01f) {
                auto& btn = simButtons[i];
                sf::Color origBase = btn.base.getFillColor();
                btn.base.setFillColor(sf::Color(origBase.r, origBase.g, origBase.b,
                    static_cast<sf::Uint8>(origBase.a * buttonAlpha)));
                btn.draw(window);
                btn.base.setFillColor(origBase);
            }
        }
    }
    ...
    // ===== Panneau d'info de la mission survolée (quand le panneau est OUVERT) =====
    if (showMissionPanel && hoveredMission && missionTimeline) {
        sf::Vector2f panelPos(window.getSize().x - 320.f, 200.f);
...
// ===== STRUCTURE INTERNE DES PLANÈTES - VERSION AVEC TEXTURES ET TEXTES AGRANDIS =====
...
void drawPlanetStructure(sf::RenderWindow& window, const sf::Font& font,
    ...
    std::vector<PlanetStructure> planetStructures = {
        ...
        {"MERCURE", sf::Color(169, 169, 169), {
           ...
           "15 329 km", "4,5 milliards d'annees", "88 jours", "74 797 000 km2",
           "3,285 x 10^23 kg", "0 satellite naturel", 0},
        ...
        {"VENUS", sf::Color(255, 198, 73), {
           ...
           "38 025 km", "4,5 milliards d'annees", "225 jours", "460 234 317 km2",
           "4,867 x 10^24 kg", "0 satellite naturel", 1},
        ...
        {"TERRE", sf::Color(100, 180, 255), {
           ...
           "40 075 km (equatorial)", "Plus de 4,5 milliards d'annees", "365,256 jours", "510 072 000 km2",
           "5,972 sextillions de tonnes", "1 satellite naturel : la Lune", 2},
        ...
        {"MARS", sf::Color(205, 92, 92), {
           ...
           "21 344 km", "4,6 milliards d'annees", "687 jours", "144 798 500 km2",
           "6,39 x 10^23 kg", "2 satellites : Phobos, Deimos", 3},
        ...
        {"JUPITER", sf::Color(218, 165, 32), {
           ...
           "439 264 km", "4,6 milliards d'annees", "11,86 ans", "61,42 milliards km2",
           "1,898 x 10^27 kg", "95 satellites connus", 4},
        ...
        }, "Celebre pour ses anneaux, densite inferieure a l'eau (elle flotterait!).",
           "378 675 km", "4,5 milliards d'annees", "29,46 ans", "42,7 milliards km2",
           "5,683 x 10^26 kg", "146 satellites connus", 5},
        ...
        {"URANUS", sf::Color(175, 238, 238), {
           ...
           "160 590 km", "4,5 milliards d'annees", "84 ans", "8,083 milliards km2",
           "8,681 x 10^25 kg", "27 satellites connus", 6},
        ...
        {"NEPTUNE", sf::Color(65, 105, 225), {
           ...
           "155 600 km", "4,5 milliards d'annees", "165 ans", "7,618 milliards km2",
           "1,024 x 10^26 kg", "16 satellites connus", 7}
    };

    const auto& planet = planetStructures[selectedPlanetIndex];

    // ===== TITRE PRINCIPAL - STYLE INFOGRAPHIE =====
    ...
    // ===== VISUALISATION DE LA STRUCTURE - POSITION REMONTÉE =====
    float structX = 280.f;
    float structY = 380.f;  // ✅ CORRIGÉ: Remonté de 450 à 380
    ...
    // ===== LÉGENDE DES COUCHES - POSITION AJUSTÉE =====
    float labelX = 530.f;
    float labelStartY = 220.f;  //  Remonté de 230 à 220
    float labelSpacing = 65.f;   // Réduit de 70 à 65 pour mieux s'adapter
    ...
    for (size_t i = 0; i < infoBoxes.size(); ++i) {
        ...
        // Titre
        sf::Text titleText(infoBoxes[i].title, font, 14);
        titleText.setPosition(boxX + boxWidth / 2.f, boxY + 15.f);
        ...
        // Valeur
        sf::Text valueText(infoBoxes[i].value, font, 14);
        valueText.setPosition(boxX + boxWidth / 2.f, boxY + 45.f);
    ...
    for (auto& b : backButtons) {
        if (b.hover) {
            sf::RectangleShape hoverGlow(sf::Vector2f(b.base.getSize().x + 20.f,
                b.base.getSize().y + 20.f));
            hoverGlow.setOrigin(10.f, 10.f);
            hoverGlow.setPosition(b.base.getPosition());
            hoverGlow.setFillColor(sf::Color(255, 100, 100, 60));
            window.draw(hoverGlow);
        }
        b.draw(window);
...
void drawSunBloom(sf::RenderWindow& window, sf::Vector2f sunPos, float time) {
    // Plusieurs couches de lueur
    for (int i = 5; i >= 0; --i) {
        float radius = 60.f + i * 15.f;
        float pulse = 1.f + 0.1f * std::sin(time * 2.f + i * 0.5f);

        sf::CircleShape bloom(radius * pulse);
        bloom.setOrigin(radius * pulse, radius * pulse);
        bloom.setPosition(sunPos);

        sf::Uint8 alpha = static_cast<sf::Uint8>(40 / (i + 1));
        bloom.setFillColor(sf::Color(255, 200, 50, alpha));
